...
def _lag_cities_countries(data, num_cities, num_countries, n_city_lags=5, n_country_lags=5):
    lag_cities = data.groupby("utrip_id")["city_id"].shift(range(1, n_city_lags + 1), fill_value=num_cities)
    lag_cities_columns = [f"city_id_lag{i}" for i in range(1, n_city_lags + 1)]
    lag_cities.columns = lag_cities_columns

    lag_countries = data.groupby("utrip_id")["hotel_country"].shift(
        range(1, n_country_lags + 1), fill_value=num_countries
    )
    lag_countries_columns = [f"hotel_id_lag{i}" for i in range(1, n_country_lags + 1)]
...
def load_data(n_trips=None):
    ...
    data = _city_count_per_trip(data, low_frequency_city_threshold=9 * 2)
    ...
    season = pd.DataFrame({"month": range(1, 13), "season": ([0] * 3) + ([1] * 3) + ([2] * 3) + ([3] * 3)})
    data = data.merge(season, how="left", on="month")
    ...
    data, lag_cities, lag_countries = _lag_cities_countries(
        data, num_cities, num_countries, n_city_lags=5, n_country_lags=1
    )
